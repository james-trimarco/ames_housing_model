...
The studenized residuals for the most seem to have fairly constant variance about zero, suggesting a linear model is appropriate. There are a couple outliers present, which we took note of. In our modeling code, we removed outliers in key quantitative predictors. 
...
For every increase in Garage Area by one square foot, we predict sale price to increase by at least  \$2.28 and at most $33.63, with 95% confidence and keeping all other predictors fixed.
...
We then separated the data andremoved outliers. 
